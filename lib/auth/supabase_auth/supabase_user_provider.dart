...
class CurupiraSupabaseUser extends BaseAuthUser {
  CurupiraSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> curupiraSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CurupiraSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
